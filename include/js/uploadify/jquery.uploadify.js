		...
		$.extend($.fn,{
			fileUpload:function(options) {
				if (flashVer >= 9) {
					...
					data += '&tableElement=' + settings.tableElement;
					data += '&idElement=' + settings.idElement;
					...
					$(this).bind("rfuError", {'action': settings.onError}, function(event, queueID, fileObj, errorObj) {
						if (event.data.action(event, queueID, fileObj, errorObj) !== false) {
							$("#" + $(this).attr('id') + queueID + " .fileName").text(errorObj.type + " Error " + errorObj.status + " - " + fileObj.name);
					...
					$(this).bind("rfuComplete", {'action': settings.onComplete}, function(event, queueID, fileObj, response, data) {
						if (event.data.action(event, queueID, fileObj, unescape(response), data) !== false) {
							$("#" + $(this).attr('id') + queueID).fadeOut(1500, function() { $("#" + $(this).attr('id') + queueID).remove()});
							$("#" + $(this).attr('id') + queueID + " .percentage").text(' - Completed');
							$("#ajax-response").load(settings.ajaxFile, {"post" : "true", "table" : settings.table, "act" : "edit", "tableElementPhoto" : settings.tableElement, "idElementPhoto" : settings.idElement, "photo" : fileObj.name, "description" : fileObj.name});
						}
					});
