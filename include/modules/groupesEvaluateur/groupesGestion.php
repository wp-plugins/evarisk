 ...
 */
	...
	if ( !current_user_can('Evarisk_:_gerer_groupes_evaluateurs') )
	{
		wp_die(__('Cheatin&#8217; uh?'));
	}

	$evaGroupAction = false;
	if(isset($_REQUEST['act']) && ($_REQUEST['act'] == 'addevaUserEvaluatorGroup') && !$createOK)
	{
		$evaGroupAction = true;
		$actionMessage = __('Le groupe n\'&agrave; pas pu &ecirc;tre cr&eacute;&eacute;','evarisk');
	}
	if(isset($_REQUEST['act']) && ($_REQUEST['act'] == 'modevaUserEvaluatorGroup') && !$updateOK)
	{
		$evaGroupAction = true;
...
<fieldset class="mainUserEvaluatorGroupInfosFieldset" >
	<legend class="titleDiv" ><?php echo __('Informations principales','evarisk'); ?></legend>
	<table class="form-table">
		<tr class="form-field form-required">
			<th scope="row"><label for="evaluator_group_name"><?php _e('Nom du groupe'); ?><span class="description"><?php _e('(required)'); ?></span></label></th>
			<td><input name="evaluator_group_name" type="text" id="evaluator_group_name" value="<?php echo $evaluator_group_name; ?>" /></td>
		</tr>

		<tr class="form-field">
			<th scope="row"><label for="evaluator_group_description"><?php _e('Description du groupe'); ?></label></th>
			<td><textarea rows="5" cols="5" id="evaluator_group_description" name="evaluator_group_description" ><?php echo $evaluator_group_description; ?></textarea></td>
		</tr>
	</table>
</fieldset>
						...
						<div class="ui-widget-content">
							<ul>
								<?php 
									...
									if( is_array($userAlreadyAffected) && ((count($userAlreadyAffected) - 1) == (count($listExistingUser) - 1)))
									{
										$elementStyle = '';
									}
								?>
