...
class wpdigi_dtransfert_ctr {
	...
	/**
	 * Declare an array for making correspondance between evarisk old types and wordpress new type
	...
	function __construct( $with_menu = false ) {
		$this->post_type[ TABLE_TACHE ] = 'wpeomtm-tasks';
		$this->post_type[ TABLE_ACTIVITE ] = 'wpeomtm-tasks';
	 ...
	 */
	function admin_menu() {
		add_management_page( __( 'Manage datas transfert from digirisk V5.X', 'wp-digi-dtrans-i18n' ), __( 'Digirisk - transfer', 'wp-digi-dtrans-i18n' ), 'manage_options', 'digirisk-transfert', array( &$this, 'tools_page' ));
	}
	...
	/**
	 * DISPLAY - The main page for transfering data form old digirisk database table to wordpress database table
	 *
	 * @param string $current_content The default parameter for wordpress filter - Unused in this case
	 * @param string $main_element_type The element type to display thrgough filter
	 *
	 */
	function display_transfert_interface( $current_content, $main_element_type ) {
		global $wpdb;
		...
		switch ( $main_element_type ) {
			case TABLE_TACHE:
					...
					$file_to_include = 'tasks';
				break;

			case TABLE_GROUPEMENT:
					$sub_element_type = TABLE_UNITE_TRAVAIL;
					$file_to_include = 'societies';
				break;
		}
	 ...
	 */
	function tools_page() {
		require( self::get_template_part( DIGI_DTRANS_DIR, DIGI_DTRANS_TEMPLATES_MAIN_DIR, "backend", "tools" ) );
	}
		...
		switch ( $_POST[ 'element_type' ] ) {
			case TABLE_TACHE:
				...
				if ( !empty( $distinct_users ) ) {
					require( self::get_template_part( DIGI_DTRANS_DIR, DIGI_DTRANS_TEMPLATES_MAIN_DIR, "backend", "transfert", "user-options" ) );
				}
			...
			if ( $main_element_already_moved < $nb_element_to_transfert->main_element_nb ) {
				...
				foreach ( $first_level_elements as $element ) {
					if ( !empty( $element ) ) {
						...
						$this->transfer_log[ 'creation' ][ 'failed' ][] = serialize( $element );
					}
				}
				...
				$transfered_element = get_option( 'wpeotm-digirisk-dtransfert', array() );
				...
				if ( !empty( $children ) ) {
					foreach ( $children as $child ) {
						...
						if ( !empty( $new_children_id ) && is_int( $new_children_id ) ) {
							$this->transfer_log[ 'creation' ][ 'created' ][ $element_id ][ $children_type ][] = $new_children_id;
							$response[ 'element_nb_treated' ]++;
						}
						else {
							$this->transfer_log[ 'creation' ][ 'failed' ][ $element_id ][ $children_type][] = $new_children_id;
						}
					}
				}
				$the_count_response = $this->buil_element_transfert_number( $element_type );
	...
	function transfer( $element_type_being_transfered, $element_being_transfered, $element_parent = null ) {
		...
		$digirisk_transfer_options = get_option( 'wpeotm-digirisk-dtransfert', array() );
		...
		if ( empty( $digirisk_transfer_options ) || empty( $digirisk_transfer_options[ $element_type_being_transfered ] ) || !in_array( $element_being_transfered->id, $digirisk_transfer_options[ $element_type_being_transfered ]) ) {
			...
			if ( is_int( $element_id ) ) {
				...
				update_option( 'wpeotm-digirisk-dtransfert', $digirisk_transfer_options );
				...
				if ( property_exists( $element_being_transfered, 'limiteGauche') ) {
					...
					foreach ( $sub_elements as $element ) {
						...
						if ( !empty( $new_children_id ) && is_int( $new_children_id ) ) {
							$this->transfer_log[ 'creation' ][ 'created' ][ $element_id ][ $element_type_being_transfered ][] = $new_children_id;
						}
						else {
							$this->transfer_log[ 'creation' ][ 'failed' ][ $element_id ][ $element_type_being_transfered ][] = $new_children_id;
						}
					}
				}
			}
